...
class ExpandedLeadView(QDialog):
    ...
    def update_plot(self):
        ...
        try:
            ...
            # Add synthetic reflection padding when the visible window touches the
            # newest or oldest samples. This prevents display-filter ringing at the
            # graph edges, especially the live right edge seen in the expanded view.
            left_missing = max(0, pad_samples - (start_idx - padded_start))
            right_missing = max(0, pad_samples - (padded_end - end_idx))
            if left_missing > 0 or right_missing > 0:
                pad_mode = 'reflect' if len(padded_signal) > 1 else 'edge'
                padded_signal = np.pad(
                    padded_signal,
                    (left_missing, right_missing),
                    mode=pad_mode,
                )
                visible_offset += left_missing
